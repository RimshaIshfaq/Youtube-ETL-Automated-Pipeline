...
results = [tuple(row) for row in all_analytics_data]
...
def write_results_to_sheet(sheet_id, sheet_name, data, sheets_client):
    ...
    for row in data:
        (
            video_id, views, watch_time, avd, comments, likes, shares,
    ...
    if update_data:
        ...
        print(f"✅ Successfully updated {len(update_data)} rows in Google Sheets.")
    else:
        print("⚠️ No data to update.")

write_results_to_sheet(SHEET_ID, SHEET_NAME, results, sheets_client)
